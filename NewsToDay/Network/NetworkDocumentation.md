...
2. **`DefaultNetworkService`** — это служба, которая берет запрос и отправляет его в интернет. Она также получает ответ и передает его обратно тебе.
...
Теперь, когда у нас есть запрос, мы можем его отправить через службу сети (`DefaultNetworkService`).
...
Для загрузки картинок используем `ImageClient`. Он может скачать изображение по URL и сохранить его в кэш, чтобы потом быстро его показывать.
